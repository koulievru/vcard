...
(function vCardFormatter() {
	...
	module.exports = {
		...
		getFormattedString: function(vCard) {
			...
			if (vCard.socialIOSUrls) {
				let index = 0;
				console.log("TEST SOCIAL IOS");
				for (var key in vCard.socialIOSUrls) {
					index++;
					if (vCard.socialIOSUrls.hasOwnProperty(key) &&
						vCard.socialIOSUrls[key]) {
						formattedVCardString += 'item' + index + '.URL:' + vCard.socialIOSUrls[key] + nl();
